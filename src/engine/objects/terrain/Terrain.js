...
export default class Terrain {
  ...
  static fromImage (src, options) {
    return new Promise(function (resolve, reject) {
      ...
      img.onload = function () {
        ...
        let terrain = new Terrain(width, height, options)
  ...
  async build () {
    ...
    var texture = textureLoader.load(this.options.texture)

    texture.wrapS = texture.wrapT = THREE.RepeatWrapping

    texture.repeat.set(this.width / 50, this.height / 50)
    this.geometry.computeBoundingSphere()
    this.geometry.computeVertexNormals()
    var mapHeight = new THREE.TextureLoader().load(this.options.normalMap)
    mapHeight.anisotropy = 4
    mapHeight.repeat.set(4000, 4000)
    ...
    this.mesh.scale.y = 30.0

    this.rocks = buildRocks({
      terrain: this,
      texture: this.options.rocks.texture,
      normalMap: this.options.rocks.normalMap
    })
  }
  ...
  applyToPhysics (physics) {
    physics.addObject(this.rocks)
    physics.setTerrain(this)
  }
